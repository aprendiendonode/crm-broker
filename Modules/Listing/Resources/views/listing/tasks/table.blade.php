 ...
 <div class="card-box">
      ...
      <div class="table-responsive">
       <table class="foo-task  table table-bordered toggle-circle mb-0">
        ...
        <tbody>
            @if($listing->tasks)
            @forelse($listing->tasks->sortByDesc('id') as $task)
            <tr>
            <td>{{ ($loop->index + 1) }}  </td>
            <td>
            ...
            <td>
                {{-- @can('delete_listing') --}}
                    <i
                        ...
                        onclick="show_task_row({{ $listing->id }},{{ $task->id }})"
                    ...
                    <i
                        ...
                        onclick="toggle_task_edit({{ $listing->id }},{{ $task->id }})"
            ...
            <tr  class="d-none task_row_{{ $task->id }}_{{ $listing->id }}" >
                <td colspan="6">

                    @include('listing::listing.tasks.task_row')

                </td>
            </tr>

            <tr  class="d-none task_edit_{{ $task->id }}_{{ $listing->id }}" >
                <td colspan="6">
                    @include('listing::listing.tasks.edit')
                </td>
            </tr>
                    ...
                    <div class="modal-content">
                        <div class="modal-body p-4">
                            <div class="text-center">
                                ...
                                <form action="{{ url('listing/delete-listing-tasks') }}" method="post">
                                    @csrf
                                    <input  type="hidden"  name="task_id" value="{{ $task->id }}">
                                    <input  type="hidden"  name="task_listing_id" value="{{ $listing->id }}">

                                    <button type="submit" formaction="{{ url('listing/delete-listing-tasks')}}"class="btn btn-danger my-2">@lang('agency.confirm_delete')</button>
