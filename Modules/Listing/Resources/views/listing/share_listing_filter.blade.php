<div class="mb-2">
    ...
    <form method="GET" autocomplete="off"
          @if( !request()->has('purpose')) style="display: none;opacity:0;transition:0.7s"
          @endif id="advanced_filter_form">
        ...
        <div class="row">
            <div class="col-md-6 col-lg-4">
                ...
                <div class="col-sm-12">
                    <div class="form-group d-flex align-items-center" style="height: 64px;">
                        ...
                        <div style="display:flex;">
                            ...
                            <div class="radio mr-3">
                                <input type="radio" name="purpose" id="searchRentRadio1" value="rent"
                                       @if(request()->has('purpose') && request('purpose') == 'rent') checked @endif>
                                <label for="searchRentRadio1">
                                    Rent
                                </label>
                            </div>
                            <div class="radio">
                                <input type="radio" name="purpose" id="searchSaleRadio2" value="sale"
                                       @if(request()->has('purpose') && request('purpose') == 'sale') checked @endif>
                        ...
                        <select class="form-control select2" name="location" data-toggle="select2">
                            ...
                            @forelse($locations as $location)
                                <option value="{{$location}}"
                                        @if(request()->has('location') && request('location') == $location) selected @endif> {{$location}} </option>
                            @empty
                            @endforelse
                        </select>
                    </div>
                ...
                <div class="col-sm-12">
                    <label class="mb-1 font-weight-medium text-muted">@lang('listing.keywords') <i
                                title="{{trans('listing.keyword_info')}}" class="fas fa-info-circle"
                        ...
                        <select class="form-control select2" name="type" data-toggle="select2">
                            ...
                            @forelse($types as $type)
                                <option value="{{$type->id ?? ''}}"
                                        @if(request()->has('type') && request('type') == $type->id ) selected @endif> {{$type->{'name_'.app()->getLocale()} ?? ''}} </option>
                            @empty
                            @endforelse
                        </select>
                    </div>
                        ...
                        <div class="d-flex">
                            <div class="mr-1" style="flex:1">
                                <select class="form-control select2" name="min_bed" data-toggle="select2"
                                    ...
                                    <option value=""></option>
                                    <option @if(request()->has('min_bed') && request('min_bed') == 'studio') selected
                                            @endif value="studio"
                                    >@lang('listing.studio')</option>

                                    @for($i = 1;$i <= 20 ;$i++)
                                        <option @if(request()->has('min_bed') && request('min_bed') == $i) selected
                            ...
                            <div class="" style="flex:1">
                                <select class="form-control select2" name="max_bed" data-toggle="select2"
                                    ...
                                    <option value=""></option>
                                    <option @if(request()->has('max_bed') && request('max_bed') == 'studio') selected
                                            @endif value="studio"
                                    >@lang('listing.studio')</option>

                                    @for($i = 1;$i <= 20 ;$i++)
                                        <option @if(request()->has('max_bed') && request('max_bed') == $i) selected
                                                @endif value="{{ $i }}">{{ $i }}</option>
                                    @endfor
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="filter_agency" data-toggle="select2">
                            ...
                            @forelse($agencies as $agency)
                                <option value="{{$agency->receiver->id ?? ''}}"
                                        @if(request()->has('filter_agency') && request('filter_agency') == $agency->receiver->id ) selected @endif> {{$agency->receiver->{'company_name_'.app()->getLocale()} ?? ''}} </option>
                            @empty
                            @endforelse
                        </select>
                    </div>
