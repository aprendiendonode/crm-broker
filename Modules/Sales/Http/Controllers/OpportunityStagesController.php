...
class OpportunityStagesController extends Controller
{
    ...
    public function index($agency)
    {
        ...
        $opportunity_stages = OpportunityStage::where('agency_id', $agency)->where('business_id', auth()->user()->business_id)->paginate($per_page);


        return view('sales::settings.opportunitystage.index', compact('agency', 'opportunity_stages', 'business'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return back()->with(flash(trans('sales.stage_created'), 'success'));
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function update($id, Request $request)
    {
        ...
        $opportunity_stage = OpportunityStage::findorfail($id);
        DB::beginTransaction();
        try {
            $validator = Validator::make($request->all(), [
            ...
            DB::commit();
            return back()->with(flash(trans('sales.stage_updated'), 'success'));
        } catch (\Exception $e) {
    ...
    public function destroy()
    {
        ...
        OpportunityStage::findorfail(request('stage_id'))->delete();

        return back()->withInput()->with(flash(trans('sales.stage_deleted'), 'success'));
    }
}