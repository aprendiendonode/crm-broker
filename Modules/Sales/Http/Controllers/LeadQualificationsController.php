...
class LeadQualificationsController extends Controller
{
    ...
    public function index($agency)
    {
        ...
        $lead_qualifications = LeadQualifications::where('agency_id', $agency)->where('business_id', auth()->user()->business_id)->paginate($per_page);


        return view('sales::settings.leadqualifications.index', compact('agency', 'lead_qualifications', 'business'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return back()->with(flash(trans('sales.qualification_created'), 'success'));
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function update($id, Request $request)
    {
        ...
        $lead_source = LeadQualifications::findorfail($id);
        DB::beginTransaction();
        try {
            $validator = Validator::make($request->all(), [
            ...
            DB::commit();
            return back()->with(flash(trans('sales.qualification_updated'), 'success'));
        } catch (\Exception $e) {
    ...
    public function destroy()
    {
        ...
        LeadQualifications::findorfail(request('qualification_id'))->delete();

        return back()->withInput()->with(flash(trans('sales.qualification_deleted'), 'success'));
    }
}