...
class LeadPrioritiesController extends Controller
{
    ...
    public function index($agency)
    {
        ...
        $lead_priorities = LeadPriority::where('agency_id', $agency)->where('business_id', auth()->user()->business_id)->paginate($per_page);


        return view('sales::settings.leadpriorities.index', compact('agency', 'lead_priorities', 'business'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            return back()->with(flash(trans('sales.priority_created'), 'success'));
        } catch (\Exception $e) {
    ...
    public function update($id, Request $request)
    {
        ...
        $lead_source = LeadPriority::findorfail($id);
        DB::beginTransaction();
        try {
            $validator = Validator::make($request->all(), [
            ...
            DB::commit();
            return back()->with(flash(trans('sales.priority_updated'), 'success'));
        } catch (\Exception $e) {
    ...
    public function destroy()
    {
        ...
        LeadPriority::findorfail(request('priority_id'))->delete();

        return back()->withInput()->with(flash(trans('sales.priority_deleted'), 'success'));
    }
}