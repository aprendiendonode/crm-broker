            ...
            <div class="modal fade" id="add_qualification" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="myCenterModalLabel">@lang('sales.new_lead_qualification')   </h5>
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                        </div>
    
                        <div class="d-flex justify-content-center">
    
                            <p class="qualification-error-message font-weight-medium text-danger"></p>
                        </div>
                        <div class="modal-body">
                            <form  action="{{ url('sales/manage_lead_qualification') }}" method="POST" >
                                @csrf
                                ...
                                <div class="form-group">
                                    <label class="mb-1 font-weight-medium text-muted">@lang('sales.name')</label>
                                    <input type="text" class="form-control qualification_name_en" name="name_en"     placeholder="@lang('sales.name')" required>
                                </div>
                                <div class="form-group">
                                    <label class="mb-1 font-weight-medium text-muted">@lang('sales.name_ar')</label>
                                    <input type="text" class="form-control qualification_name_ar"  name="name_ar"    placeholder="@lang('sales.name_ar')" required>
                                </div>
    
                                
                                <div class="form-group text-center">
                                    <button class="btn btn-rounded btn-primary" onclick="event.preventDefault();add_lead_qualification()" >@lang('sales.confirm')</button>
                                </div>
                            </form>
                        </div>
                    </div><!-- /.modal-content -->
...
<script>
    ...
    function add_lead_qualification(){
        var name_en  =  $('.qualification_name_en').val();
        var name_ar  =  $('.qualification_name_ar').val();
        var agency   =  @json($agency);
        var business =  @json($business);
        ...
        $.ajax({
            url:"{{ url('sales/lead_qualification_from_leads')  }}",
            type:'POST',
            data:{
                name_en : name_en,
                name_ar : name_ar,
            ...
            success: function(data){
                $('.qualification-error-message').text('');
                name = '';
                if(locale == 'en'){
                    name = data.data.name_en
                } else {
                ...
                html = '<option value='+data.data.id+' >'+name+'</option>'
                $('.select_qualification_id').append(html)
                   $('.qualification_name_en').val('');
                   $('.qualification_name_ar').val('');
            ...
            error: function (error) {
                $('.qualification-error-message').text(error.responseJSON.message);
            }
        })
    }
