...
<form  action="{{ url('sales/manage_clients/assign_call/'.$client->id) }}" data-parsley-validate="" method="POST" enctype="multipart/form-data">
    <div class="row">
            @csrf
            @method('PATCH')
    
            @if($agency)
            <input type="hidden" name="call_agency_id_{{ $client->id }}" value="{{ $agency }}">
            @endif
            @if($business)
            <input type="hidden" name="call_business_id_{{ $client->id }}" value="{{ $business }}">
            @endif
        ...
        <div class="col-md-12">
            ...
            <div class="form-group ">
               ...
               <div class="d-flex justify-content-between">
                   ...
                   <div style="flex:4">
                            ...
                            <select  class="form-control select2 select_callstatus_id" name="call_status_id_{{ $client->id }}" data-toggle="select2" data-placeholder="@lang('sales.call_status')" required >
                        
                                <option value=""></option>
                                @forelse($call_status as $status)
                                <option @if(old("call_status_id_{$client->id}",$client->status_id) == $status->id) selected @endif  value="{{ $status->id}}">{{ $status->{'name_'.app()->getLocale()} }}</option>
                                @empty
            ...
            <div class="form-group mb-3  ">
                ...
                <input type="text" name="call_contact_date_{{ $client->id }}"  class="form-control basic-datepicker" value="{{ old('call_contact_date_'.$client->id,  date('Y-m-d')) }}" placeholder="@lang('sales.contact_date')" required>
            ...
            <div class="form-group mb-3">
                ...
                <div class="input-group  clockpicker"  data-placement="top" data-align="top" data-autoclose="true">
                    <input type="text" name="call_contact_time_{{ $client->id }}" class="form-control" value="{{ old('call_contact_date_{$client->id}',date('h:i') ) }}" required>
                    <div class="input-group-append">
                        <span class="input-group-text"><i class="mdi mdi-clock-outline"></i></span>
                    </div>
                </div>
            ...
            <div class="form-group mb-3  ">
                ...
                <div class="input-group">
                <input type="text" name="call_next_action_date_{{ $client->id }}"  class="form-control basic-datepicker" value="{{ old('call_next_action_date_'.$client->id) }}" placeholder="@lang('sales.next_action_date')">
                <div class="input-group-append">
                    <span class="input-group-text"><i class="mdi mdi-clock-outline"></i></span>
                </div>
            </div>                
            ...
            <div class="form-group mb-3">
                ...
                <div class="input-group clockpicker" data-placement="top" data-align="top" data-autoclose="true">
                    <input type="text" name="call_next_action_time_{{ $client->id }}"  class="form-control" placeholder="@lang('sales.next_action_time')">
                    <div class="input-group-append">
                        <span class="input-group-text"><i class="mdi mdi-clock-outline"></i></span>
                    </div>
                </div>
            ...
            <div class="form form-group">
                ...
                <label class="text-muted font-weight-medium">@lang('sales.call_breif')<i class="text-danger" style="font-size:15px;font-weight:bold">*</i></label>
                    ...
                    <textarea required class="form-control" name="call_note_{{ $client->id }}" placeholder="@lang('sales.call_breif')">{{old("call_note_{$client->id}")}}</textarea>
    ...
    <div class="d-flex justify-content-end">
        ...
        <button onclick="event.preventDefault();hide_call_div({{ $client->id }})" type="button" class="btn  btn-outline-success waves-effect waves-light">
           @lang('sales.cancel')
        </button>
                ...
                <table class="foo-call table table-bordered toggle-circle mb-0">
                    ...
                    <tbody>
                        ...
                        <td>
                            ...
                            onclick="event.preventDefault();show_call_breif_div({{ $call->id }},{{ $client->id }})"
                            data-plugin="tippy" 
                            data-tippy-placement="top-start" 
                        ...
                        <tr  class="call_breif_{{ $call->id }}_{{ $client->id }} d-none"    >
                            <td colspan="7">
                                ...
                                <div class="modal-content">
                                    <div class="modal-body p-4">
                                        <div class="text-center">
                                            ...
                                            <form action="{{ url('sales/delete-client-calls') }}" method="post">
                                                @csrf
                                                <input  type="hidden" name="call_id" value="{{ $call->id }}">
                                                <input type="hidden" name="call_client_id" value="{{ $client->id }}">
            
                                                <button type="submit" formaction="{{ url('sales/delete-client-calls')}}"class="btn btn-danger my-2">@lang('agency.confirm_delete')</button>
