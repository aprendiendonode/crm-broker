      ...
      <div class="table-responsive">
       <table class="foo-task  table table-bordered toggle-circle mb-0">
        ...
        <tbody>
            ...
            @forelse($opportunity->tasks->sortByDesc('id') as $task)
            <tr>
            <td>{{ ($loop->index + 1) }}  </td>
            <td>
            ...
            <td>
                {{-- @can('delete_opportunity') --}}
                    <i
                        ...
                        onclick="show_task_row({{ $opportunity->id }},{{ $task->id }})"
                    ...
                    <i
                        ...
                        onclick="toggle_task_edit({{ $opportunity->id }},{{ $task->id }})"
            ...
            <tr  class="d-none task_row_{{ $task->id }}_{{ $opportunity->id }}" >
                <td colspan="6">

                    @include('sales::opportunities.tasks.task_row')

                </td>
            </tr>

            <tr  class="d-none task_edit_{{ $task->id }}_{{ $opportunity->id }}" >
                <td colspan="6">
                    @include('sales::opportunities.tasks.edit')
                </td>
            </tr>
                    ...
                    <div class="modal-content">
                        <div class="modal-body p-4">
                            <div class="text-center">
                                ...
                                <form action="{{ url('sales/delete-opportunity-tasks') }}" method="post">
                                    @csrf
                                    <input  type="hidden"  name="task_id" value="{{ $task->id }}">
                                    <input  type="hidden"  name="task_opportunity_id" value="{{ $opportunity->id }}">

                                    <button type="submit" formaction="{{ url('sales/delete-opportunity-tasks')}}"class="btn btn-danger my-2">@lang('agency.confirm_delete')</button>
