<form action="{{ route('activity.tasks.update',$task->id) }}" data-parsley-validate="" method="POST"
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                <select class="selectpicker mb-0 show-tick" data-toggle="select2" name="task_type_edit_{{ $task->id }}"
                        id="task_type_edit_{{ $task->id }}" onchange="show_reference_div_edit('{{ $task->id }}')"
                        required>

                    <option value=""
                            {{ old('task_type_edit_'.$task->id, '') == '' ? 'selected' : ''}} disabled> @lang('global.pleaseSelect')</option>
            ...
            <div class="form-group">
                ...
                <select class="selectpicker mb-0 show-tick" name="status_{{$task->id}}" data-toggle="select2"
                        id="status_{{$task->id}}" required>

                    <option value=""
                            {{old('status_'.$task->id, '' ) == '' ? 'selected' : ''}} disabled> @lang('global.pleaseSelect')</option>
                    {{--<optgroup label="Open">--}}
            ...
            <input type="hidden" id="old_module_edit_{{$task->id}}"
                   value="{{ old('module_edit_'.$task->id,$task->module) ?? '' }}"/>
            <input type="hidden" id="task_id_edit" value="{{ $task->id }}"/>
            <div class="form-group" style="margin-bottom:32px">
                <div class="radio radio-info form-check-inline">
                    <input type="radio" id="lead_edit_{{$task->id}}" value="lead"
                           onchange="show_module_div_edit('lead',{{$task->id}})"
                           name="module_edit_{{$task->id}}" {{ old('module_edit_'.$task->id,$task->module) == 'lead' ? 'checked' : '' }} {{ old('module_edit_'.$task->id,null) == null ? 'checked' : '' }} >
                    <label for="lead_edit_{{$task->id}}"> @lang('sales.lead') </label>
                </div>

                <div class="radio radio-info form-check-inline">
                    <input type="radio" id="opportunity_edit_{{$task->id}}" value="opportunity"
                           onchange="show_module_div_edit('opportunity',{{$task->id}})"
                           name="module_edit_{{$task->id}}" {{ old('module_edit_'.$task->id,$task->module) == 'opportunity' ? 'checked' : '' }} >
                    <label for="opportunity_edit_{{$task->id}}"> @lang('sales.opportunity')</label>
                </div>
                <div class="radio radio-info form-check-inline">
                    <input type="radio" id="client_edit_{{$task->id}}" value="client"
                           onchange="show_module_div_edit('client',{{$task->id}})"
                           name="module_edit_{{$task->id}}" {{ old('module_edit_'.$task->id,$task->module) == 'client' ? 'checked' : '' }} >
                    <label for="client_edit_{{$task->id}}"> @lang('sales.client') </label>
                </div>
                <div class="radio radio-info form-check-inline">
                    <input type="radio" id="listing_edit_{{$task->id}}" value="listing"
                           onchange="show_module_div_edit('listing',{{$task->id}})"
                           name="module_edit_{{$task->id}}" {{ old('module_edit_'.$task->id,$task->module) == 'listing' ? 'checked' : '' }} >
            ...
            <div class="form-group" id="leads_div_edit_{{$task->id}}" style="display:none">
                ...
                <select class="form-control select2" data-toggle="select2" name="lead_id_edit_{{$task->id}}">
                    ...
                    <option value=""
                            {{old('lead_id_edit_'.$task->id, '' ) == '' ? 'selected' : ''}} disabled> @lang('global.pleaseSelect')</option>
                    @if($leads)
            ...
            <div class="form-group" id="opportunities_div_edit_{{$task->id}}" style="display:none">
                ...
                <select class="form-control select2" data-toggle="select2" name="opportunity_id_edit_{{$task->id}}">
                    ...
                    <option value=""
                            {{old('opportunity_id_edit_'.$task->id, '' ) == '' ? 'selected' : ''}} disabled> @lang('global.pleaseSelect')</option>
                    @if($opportunities)
                        @foreach($opportunities as $opportunity)
                            <option value="{{$opportunity->id }}" {{ old('opportunity_id_edit_'.$task->id,($task->module == 'opportunity') ? $task->module_id : '') == $opportunity->id ? 'selected' : '' }}>{{ $opportunity->lead && $opportunity->lead->full_name ? $opportunity->lead->full_name : ($opportunity->full_name ?? '')	 }}</option>
                        @endforeach
                    @endif
                </select>
            ...
            {{--select client--}}
            <div class="form-group" id="clients_div_edit_{{$task->id}}">
                ...
                <select class="selectpicker mb-0 show-tick" data-toggle="select2" name="client_id_edit_{{$task->id}}">
                    ...
                    <option value=""
                            {{old('client_id', '' ) == '' ? 'selected' : ''}} disabled> @lang('global.pleaseSelect')</option>
                    @if($clients)
                        @foreach($clients as $client)
                            <option value="{{$client->id }}" {{ old('client_id_edit_'.$task->id,($task->module == 'client') ? $task->module_id : '') == $client->id ? 'selected' : '' }}>{{$client->name	 }}</option>
                        @endforeach
                    @endif
                </select>
            ...
            {{--select listing--}}
            <div class="form-group" id="listings_div_edit_{{$task->id}}">
                ...
                <select class="selectpicker mb-0 show-tick" data-toggle="select2" name="listing_id_edit_{{$task->id}}">
                    ...
                    <option value=""
                            {{old('listing_id', '' ) == '' ? 'selected' : ''}} disabled> @lang('global.pleaseSelect')</option>
                    @if($listings)
                        @foreach($listings as $listing)
                            <option value="{{$listing->id }}" {{ old('listing_id_edit_'.$task->id,($task->module == 'listing') ? $task->module_id : '') == $listing->id ? 'selected' : '' }}>{{$listing->title	 }}</option>
                        @endforeach
                    @endif
                </select>
                ...
                <div class="form-group custom-toggle">
                    <div class="d-flex justify-content-between">
                        <label class="mb-1 font-weight-medium text-muted"
                               for="note_en">@lang('activity.tasks.last_note')</label>

                        <input onchange="toggle_edit_note('{{ $task->id }}')" class="notes_edit_{{ $task->id }}"
                               type="checkbox"
                    ...
                    <div class="notes_edit_en_{{ $task->id }}">
                        ...
                        <textarea id="notes_edit_en_{{ $task->id }}"
                                  name="edit_notes_en_{{ $task->id }}">{{old('edit_notes_en_'.$task->id,$task->notes && $task->notes->last() && $task->notes->last()->notes_en ? $task->notes->last()->notes_en : '')}}</textarea>
                    </div>
                    <div class="notes_edit_ar_{{ $task->id }} d-none">

                        <textarea id="notes_edit_ar_{{ $task->id }}"
                                  name="edit_notes_ar_{{ $task->id }}">{{old('edit_notes_ar_'.$task->id,$task->notes && $task->notes->last() && $task->notes->last()->notes_ar ? $task->notes->last()->notes_ar : '')}}</textarea>
                    </div>
        ...
        <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 " data-toggle="select2" name="staff_edit_{{$task->id}}[]"
                        id="staff_{{$task->id}}" multiple required>
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" name="deadline_edit_{{$task->id}}" id="deadline_edit_{{$task->id}}"
                       value="{{ old('deadline_edit_'.$task->id,$task->deadline ?? '')}}"
                       class="form-control basic-datepicker" placeholder="@lang('activity.tasks.deadline')" required>
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <div class="input-group clockpicker" data-placement="top" data-align="top" data-autoclose="true">
                    <input type="text" class="form-control" name="time_edit_{{$task->id}}" id="time_edit_{{$task->id}}"
                           value="{{ date('H:i', strtotime( old('time_edit_'.$task->id,$task->time ?? ''))) }}"
                           required>
            ...
            <div class="custom-control custom-checkbox">
                <input type="checkbox" class="custom-control-input" name="custom_reminder_edit_{{$task->id}}"
                       id="custom_reminder_edit_{{$task->id}}"
                       {{ old('custom_reminder_edit_'.$task->id,$task->custom_reminder ?? '') == 'on' ? 'checked' : '' }}
                       onchange="show_custom_reminder_edit('{{$task->id}}')">

                <label class="custom-control-label"
                       for="custom_reminder_edit_{{$task->id}}">@lang('activity.tasks.custom_reminder')</label>
            </div>
            ...
            <div class="form-group mt-2" id="div_custom_reminder_edit_{{$task->id}}" style="display: none">
                ...
                <div class="d-flex">
                    <div class="" style="flex:2">
                        <select class="selectpicker mb-0 show-tick" name="period_reminder_edit_{{$task->id}}"
                                id="period_reminder_edit_{{$task->id}}">
                    ...
                    <div class="px-2" style="flex:2">
                        <select class="selectpicker mb-0 show-tick" name="type_reminder_edit_{{$task->id}}"
                                id="type_reminder_edit_{{$task->id}}">
                            <option selected disabled>Select</option>
                    ...
                    <div class="" style="flex:2">
                        <input type="number" class="form-control" name="type_reminder_number_edit_{{$task->id}}"
                               id="type_reminder_number_edit_{{$task->id}}"
                               value="{{old('type_reminder_number_edit_'.$task->id,$task->type_reminder_number ?? 0)}}">
                    </div>
                </div>
            </div>
    ...
    <div class="d-flex justify-content-end">
        ...
        <button onclick="event.preventDefault();hide_edit_div({{ $task->id }})" type="button"
